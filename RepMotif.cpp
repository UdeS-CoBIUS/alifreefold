...
RepNmotif::RepNmotif(string i_PathFileInput,	int maxLevelNmotifs) {
	// TODO Auto-generated constructor stub
    this->maxLevelNmotifs = maxLevelNmotifs;
	this->isCircular = false;

    readFileandParseSequenceStructure(i_PathFileInput, headers, structures, sequences,isCircular,minNumberOfSubOpt);
    if (headers.size()<2)
...
void RepNmotif::readFileandParseSequenceStructure(string filePath,
	...
	while (getline(myfile, line)) {
		if (line[0] == '>') {
            ...
            tempLineHeader.erase( remove( tempLineHeader.begin(), tempLineHeader.end(), ' ' ), tempLineHeader.end() );
